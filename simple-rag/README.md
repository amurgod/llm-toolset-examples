...
## Optional: Run ChromaDB Server

The project uses ChromaDB as an embedded vector store by default, which means it runs locally within the application. However, you can also run ChromaDB as a separate server for better performance or to access it from multiple applications.

### Install ChromaDB CLI
First, install the ChromaDB CLI tool:
```bash
pip install chromadb
```

### Start ChromaDB Server
Run the ChromaDB server pointing to your existing database:
```bash
chroma run --path chroma_db
```

This will start a ChromaDB server on `http://localhost:8000` with your existing vector database.

### Access ChromaDB UI
Once the server is running, you can access the ChromaDB UI at:
- **ChromaDB Dashboard**: http://localhost:8000
- **API Documentation**: http://localhost:8000/docs

### Alternative: Use Custom ChromaDB UI
If you prefer a more integrated experience, the project includes a custom Streamlit-based ChromaDB UI:
```bash
streamlit run chroma_ui.py
```
